    ...
    if search_button and university_name:
        with st.spinner('Searching university information...'):
            ...
            if best_match:
                ...
                if uni_system.check_data_completeness(uni_data):
                    st.success(f"✅ Found complete information in cache")
                else:
                    st.info("⚠️ Some information in cache is incomplete or marked as N/A. Fetching fresh data from AI...")
                    with st.spinner('Fetching from AI...'):
                        fresh_data = uni_system.fetch_university_details(uni_name)
                        # Check if we got valid data
                        if fresh_data and fresh_data.get('academic_info', {}).get('nirf_rank') != "Not available":
                            uni_data = fresh_data
                            uni_system.save_to_master_json(uni_name, uni_data)
                            st.success("✅ University details updated from AI")
                        else:
                            st.warning("⚠️ Using cached data as AI fetch failed")
            else:
                # Fetch from internet when NOT found in cache
                st.info("🔄 University not found in cache. Fetching from AI...")
                with st.spinner('Fetching from AI...'):
                    uni_data = uni_system.fetch_university_details(university_name)
                    uni_name = university_name
                    
                    # Check if we got valid data (not an error response)
                    if uni_data and uni_data.get('academic_info', {}).get('nirf_rank') != "Not available":
                        uni_system.save_to_master_json(uni_name, uni_data)
                        st.success("✅ University details fetched from AI")
                    else:
                        st.error("❌ Could not fetch university information from AI")
                        return
