...
# # # Configuration - Use relative path for GitHub
# # MASTER_JSON_FILE = "universities_master.json"
...
# # CRITICAL: 
# # - If university is NOT in India, provide QS World Ranking instead of NIRF rank
# # - Include BOTH nirf_rank (for Indian univs) and qs_rank (for non-Indian univs)
...
# #         "qs_rank": "QS World Rank for non-Indian univs, otherwise N/A",
# #         "nirf_rank_numeric": 0,
# #         "courses": {{
...
# #         country = uni_data.get('country', 'India')
# #         if country != 'India':
# #             qs_rank = uni_data.get('academic_info', {}).get('qs_rank', 'N/A')
# #             # Try to extract numeric value from rank string
# #             rank_str = str(qs_rank)
# #             req_nirf = 0
# #             numbers = re.findall(r'\d+', rank_str)
# #             if numbers:
# #                     req_nirf = int(numbers[0])
# #         else: 
...
# #         country = uni_data.get('country', 'India')
# #         if country == 'India':
# #             req_rank = uni_data.get('academic_info', {}).get('nirf_rank_numeric', 0)
        
# #             fig.add_trace(
# #                 go.Bar(x=[uni_name[:15], 'SASTRA'], y=[req_rank, sastra_nirf],
# #                     marker_color=['#3498db', '#e74c3c'],
# #                     text=[req_rank, sastra_nirf], textposition='outside'),
# #                 row=1, col=1
# #             )
# #         else:
# #             fig.add_trace(
# #             go.Bar(x=['SASTRA (NIRF)'], y=[sastra_nirf],
# #                    marker_color=['#e74c3c'],
# #                    text=[sastra_nirf], textposition='outside'),
# #             row=1, col=1
# #                          )
...
# #                 if uni_data.get('country', 'India') != 'India':
# #                     rank_display = acad.get('qs_rank', 'N/A')
# #                     st.metric("QS World Rank", rank_display)
# #                 else: 
# #                      rank_display = acad.get('nirf_rank', 'N/A')
# #                      st.metric("NIRF Rank", rank_display)
...
#     def fetch_university_details(self, university_name: str) -> Dict[str, Any]:
#         prompt = f"""You are a university research assistant with REAL-TIME INTERNET ACCESS. 
# Search for latest and CORRECT information about "{university_name}".
...
# - If university is NOT in India, provide QS World Ranking instead of NIRF rank
# - Include BOTH nirf_rank (for Indian univs) and qs_rank (for non-Indian univs)
# - IMPORTANT: For Indian universities, ensure you fetch the CORRECT and ACCURATE NIRF ranking from official NIRF website or reliable sources. Do not provide wrong or approximate rankings.
...
#         <span>Created By: Dr. K.R. Sekar</span> | 
#         <span>Dr. S. Ramesh Kumar</span> | 
#         <span>Dr. V. Venkatraman</span>
#     </div>
#     """, unsafe_allow_html=True)
...
def main():
    ...
    if st.session_state.show_results and st.session_state.uni_data:
        ...
        # Create tabs for different sections - no emojis
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["Academic", "Research", "Sports & Cultural", "Events", "Comparison Graphs"])
        ...
        with tab2:
            ...
            with col2:
                st.markdown('<div class="info-card">', unsafe_allow_html=True)
                st.markdown("**Centralized Facilities**")
                for facility in research.get('centralized_facilities', [])[:7]:
                    st.markdown(f"- {facility}")
                st.markdown('</div>', unsafe_allow_html=True)
